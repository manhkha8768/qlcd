...
function accessible(req,row){
    return (!row.don_vi_nguon_id || duocThaoTacDonVi(req.session.nguoiDung,row.don_vi_nguon_id)) &&
...
r.post('/',coMaQuyenNay('asset_ledger.create'),(req,res)=>{
    ...
    for(const line of b.chi_tiet){
        ...
        if(!accessible(req,line))return res.status(403).json({loi:'Đơn vị ngoài phạm vi dữ liệu'});
...
r.post('/:id/post',coMaQuyenNay('asset_ledger.post'),(req,res)=>{
    try{db.transaction(()=>{
        ...
        if(tx.trang_thai==='POSTED')return;
