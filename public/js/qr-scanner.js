let qrStream=null,qrFrame=null,qrDangXuLy=false;
async function mhQrScan(el){dungQrCamera();el.innerHTML=`<div class="dau-trang"><div><div class="eyebrow">QR thiết bị</div><h2>Quét bằng camera</h2><div class="phu">Mã được kiểm tra lại trên server và tuân Data Scope</div></div></div>
 ...
 <div class="than-the"><button class="chinh-nut" id="qr-start" onclick="batDauQrCamera()">Bật camera</button> <button onclick="dungQrCamera()">Dừng</button></div></div>
...
async function batDauQrCamera(){try{if(!navigator.mediaDevices?.getUserMedia)throw new Error('Thiết bị không hỗ trợ truy cập camera');
 qrStream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:'environment'}},audio:false});const video=document.getElementById('qr-video');video.srcObject=qrStream;await video.play();
 ...
 }catch(e){dungQrCamera();bao(e.message,'loi');}}
