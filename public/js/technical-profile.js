...
let hsktData=null;let hsktTab='thong-so';
...
async function moHskt(id,tab='thong-so'){
 hsktData=await api('/technical-profiles/devices/'+id);hsktTab=tab;const d=hsktData,o=d.overview;const el=document.getElementById('chinh');
...
function veTabHskt(){const d=hsktData,o=d.overview,el=document.getElementById('hskt-tab');
 ...
 else if(hsktTab==='cau-tao')el.innerHTML=`<div class="bao canh-bao">Cây cấu tạo đang đọc từ dữ liệu hiện hữu. Chuẩn hóa cây canonical thuộc TASK 8.</div>${bangHskt(['Mã cụm','Tên','Cấp'],d.components,x=>[x.ma_cum,x.ten,x.cap])}`;
