async function mhWorkInbox(el) {
    ...
    const card = x => `<article class="work-card ${['URGENT','STOP_PRODUCTION'].includes(x.priority)?'urgent':''}">
        <div><div class="eyebrow">${esc(x.item_type==='TECHNICAL_WORK_ORDER'?'Công việc kỹ thuật':x.item_type==='REPAIR_MINUTE'?'Biên bản sửa chữa':'Giao dịch tài sản')}</div>
        ...
        <div class="work-actions">${x.action_kind==='repair_minute'?`<button class="chinh-nut" onclick="viewRepairMinute('${x.id}')">Mở biên bản</button>`:x.action_kind==='technical'?`
    ...
    el.innerHTML=`<div class="dau-trang"><div><div class="eyebrow">Work Inbox</div><h2>Việc của tôi & Phê duyệt</h2>
        ...
        <div id="vung-bao"></div><div class="thanh-loc"><input id="inbox-q" class="lon" placeholder="Tìm mã, nội dung, thiết bị, đơn vị…" value="${esc(route.q||'')}"><select id="inbox-type"><option value="">Mọi loại việc</option><option value="TECHNICAL_WORK_ORDER">Work Order</option><option value="REPAIR_MINUTE">Biên bản sửa chữa</option><option value="ASSET_TRANSACTION">Giao dịch tài sản</option></select><button onclick="locWorkInbox()">Lọc</button></div>
