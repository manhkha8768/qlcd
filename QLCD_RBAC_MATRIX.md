...
TASK 22: `notification.view` cấp cho các vai trò nhưng kết quả luôn lọc theo unit scope, owner hoặc delivery cá nhân; data-quality case cấp Công ty không rò sang PX. `notification.triage` cho admin/CĐVT/PX, nhưng PX chỉ mutation case mình được giao. `notification.assign` và `notification.manage` chỉ cấp admin/CĐVT để phân công, escalation, cấu hình rule và chạy job. Người chỉ xem không được acknowledge/start/resolve. Đổi rule và mọi transition giữ actor/reason trong event bất biến.

TASK 23: `report.view` cấp cho admin/CĐVT/PX/người xem và luôn áp dụng data scope server-side. `report.export.excel`, `report.export.pdf`, `report.print` cấp admin/CĐVT/PX; người xem chỉ đọc màn hình. `report.audit` chỉ cấp admin/CĐVT để xem actor, filter/scope và hash của lần xuất. Filter `don_vi_id` ngoài scope trả 403. API report/export legacy bị chặn với vai trò PX; PX phải dùng Report Center canonical có scope.

TASK 24: route registry kiểm kê 394 method/path; chỉ đăng nhập được public rõ ràng, mọi API còn lại phải có session/role/permission policy. Thiếu policy làm acceptance test thất bại và chặn startup production. Hai endpoint danh mục File mẫu trước đây chỉ dựa vào dữ liệu công khai nội bộ nay yêu cầu đăng nhập. Middleware UI không thay thế kiểm tra server; Origin/Fetch Metadata chỉ là lớp chống CSRF bổ sung phía trước RBAC/data scope.

TASK 25: `operations.view` cấp admin và CĐVT để xem metrics/readiness/error queue; `operations.manage` chỉ cấp admin để resolve lỗi với ghi chú bắt buộc. Liveness/readiness không trả dữ liệu nghiệp vụ nên public cho orchestrator; metrics, stack fingerprint và actor luôn yêu cầu xác thực. Load test chuẩn chỉ gọi GET và không thay đổi dữ liệu.

TASK 26: staging chỉ kích hoạt `uat_admin`, `uat_cdcty`, `uat_px01` và `uat_px02` (nếu chọn đủ hai PX). Hai tài khoản PX bị giới hạn đúng một đơn vị; admin xem toàn staging và CĐVT dùng permission hiện hành. Tất cả tài khoản nguồn bị đổi định danh, vô hiệu hóa và không thể đăng nhập. Tài khoản UAT không bao giờ được tạo trên database nguồn.

TASK 27: Quick Tunnel không là authentication hoặc authorization layer. Anonymous denial, session validation, permission checks và unit scope đều vẫn do Express kiểm soát cho từng request; connector chỉ chuyển tiếp transport tới staging. Không có role nào được cấp thêm quyền vì URL tunnel, và URL không được công bố như production. Staging/UAT accounts and scope remain those of TASK 26; production credentials must never be used.
