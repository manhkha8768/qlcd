...
const { migrationChecksum, matchesMigrationChecksum } = require('./migration-checksum');
const { passwordLengthError } = require('./password-policy');
...
function applyMigrations(db, migrationsDir = path.join(__dirname, '..', 'db')) {
    ...
    for (const file of files) {
        ...
        if (applied) {
            if (!matchesMigrationChecksum(applied.checksum_sha256, sql)) throw new Error(`Migration đã bị thay đổi: ${file}`);
            continue;
        }
        db.exec(sql);
...
async function createUatStaging(options) {
    ...
    const loiDoDai = passwordLengthError(password);
