...
function physicalPathForPotential(value, cwd) {
    const missingSegments = [];
    let candidate = path.resolve(cwd, value);
    while (true) {
        try {
            const physicalAncestor = fs.realpathSync(candidate);
...
function isSameOrDescendant(candidate, ancestor) {
    const comparableCandidate = normalized(candidate, path.parse(candidate).root);
    const comparableAncestor = normalized(ancestor, path.parse(ancestor).root);
    const relative = path.relative(comparableAncestor, comparableCandidate);
...
function resolveStagingConfig(env = process.env, cwd = process.cwd()) {
    ...
    const physicalContextPath = fs.realpathSync(cwd);
    const physicalStagingPaths = [
        ['QLCD_STAGING_DB', fs.realpathSync(dbPath)],
        ['QLCD_STAGING_UPLOAD', physicalPathForPotential(uploadPath, cwd)],
        ['QLCD_STAGING_BACKUP_DIR', physicalPathForPotential(backupPath, cwd)]
    ];
    for (const [name, physicalStagingPath] of physicalStagingPaths) {
        if (isSameOrDescendant(physicalStagingPath, physicalContextPath)) {
            throw new Error(`${name} must be outside the Docker build context`);
        }
    }
    ...
    return {
        secret, dbPath, uploadPath, backupPath, port: rawPort, evidencePath,
        contextPath: physicalContextPath,
        productionPaths: productionPaths.map(([name, value]) => ({ name, value: path.resolve(cwd, value) }))
    };
}

function finalizeStagingConfig(config, cwd = process.cwd()) {
    const contextPath = config.contextPath || fs.realpathSync(cwd);
    const sourceFd = fs.openSync(config.dbPath, 'r');
    let snapshotRoot;
    try {
        const sourceIdentity = fs.fstatSync(sourceFd);
        const dbPath = fs.realpathSync(config.dbPath);
        const pathIdentity = fs.statSync(dbPath);
        if (sourceIdentity.dev !== pathIdentity.dev || sourceIdentity.ino !== pathIdentity.ino) {
            throw new Error('QLCD_STAGING_DB changed during physical validation');
        }
        const uploadCandidate = physicalPathForPotential(config.uploadPath, cwd);
        const backupCandidate = physicalPathForPotential(config.backupPath, cwd);
        for (const [name, candidate] of [
            ['QLCD_STAGING_DB', dbPath],
            ['QLCD_STAGING_UPLOAD', uploadCandidate],
            ['QLCD_STAGING_BACKUP_DIR', backupCandidate]
        ]) {
            if (isSameOrDescendant(candidate, contextPath)) {
                throw new Error(`${name} must be outside the Docker build context`);
            }
        }
        fs.mkdirSync(uploadCandidate, { recursive: true });
        fs.mkdirSync(backupCandidate, { recursive: true });
        const finalized = {
            ...config,
            dbPath,
            uploadPath: fs.realpathSync(uploadCandidate),
            backupPath: fs.realpathSync(backupCandidate)
        };
        const stagingPaths = [
            ['QLCD_STAGING_DB', finalized.dbPath],
            ['QLCD_STAGING_UPLOAD', finalized.uploadPath],
            ['QLCD_STAGING_BACKUP_DIR', finalized.backupPath]
        ];
        for (let index = 0; index < stagingPaths.length; index += 1) {
            const [name, stagingPath] = stagingPaths[index];
            for (const [otherName, otherPath] of stagingPaths.slice(index + 1)) {
        ...
        for (const production of config.productionPaths || []) {
            const productionPath = physicalPathForPotential(production.value, cwd);
            for (const [stagingName, stagingPath] of stagingPaths) {
                if (normalized(productionPath, cwd) === normalized(stagingPath, cwd)) {
                    throw new Error(`staging path must not match production (${stagingName} conflicts with ${production.name})`);
                }
            }
        }
        snapshotRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'qlcd-staging-seed-'));
        const snapshotPath = path.join(snapshotRoot, 'qlcd.db');
        const targetFd = fs.openSync(snapshotPath, 'wx', 0o600);
        try {
        ...
        if (after.size !== sourceIdentity.size || after.mtimeMs !== sourceIdentity.mtimeMs || after.ctimeMs !== sourceIdentity.ctimeMs) {
            throw new Error('Staging database changed while creating the seed snapshot');
        }
        return { ...finalized, dbSnapshotPath: snapshotPath, dbSnapshotRoot: snapshotRoot };
    } catch (error) {
...
function createController({ runPreflight, runCompose, buildImage = async () => null, finalizeConfig = async config => config, seedData = async () => {}, fetchImpl, readLogs, writeEvidence, now = Date.now, commit = 'unknown', sleep = ms => new Promise(resolve => setTimeout(resolve, ms)), timeoutMs = POLL_TIMEOUT_MS, scheduleTimeout: timer = createTimeout }) {
    ...
    async function ready(url, step, requestOptions = {}) {
        return poll({
            step,
            now,
            sleep,
    ...
    return {
        async start(config) {
            ...
            const image = await buildImage({ config });
            if (image) config = { ...config, image };
            await runCompose({ action: 'config', args: ['config', '--quiet'], config });
            await runCompose({ action: 'app-create', args: ['create', '--force-recreate', 'qlcd-staging'], config });
            config = await finalizeConfig(config);
            await seedData({ config });
            await runCompose({ action: 'app-start', args: ['start', 'qlcd-staging'], config });
...
module.exports = { resolveStagingConfig, finalizeStagingConfig, parseQuickTunnelUrl, createEvidence, createController };
