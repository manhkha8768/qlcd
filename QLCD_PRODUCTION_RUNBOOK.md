...
Prerequisites: Docker Engine and Docker Compose must be available to the operator; the tracked working tree must be clean; a staging database file must already exist; staging upload and backup directories must be distinct; and none of the staging paths may be a production database, upload, or backup path. All staging data must be physically outside the repository checkout. Use only an approved, anonymized external staging/UAT copy; never use production data with this compose file.
...
2. Run `npm run staging:tunnel:start`. The wrapper validates isolation, runs preflight, captures one commit, exports that exact commit into a temporary directory outside the checkout, builds the image, and pins Compose to its content-addressed image ID. For first initialization it requires all three named volumes to be empty, copies a stable file-descriptor snapshot of the staging database, initializes empty upload/backup volumes, then writes all seed markers. Any uncertain or partial volume state fails closed. Host uploads/backups are deliberately not copied into Quick Tunnel staging. The Compose file has neither `build:` nor host-data bind mounts.
...
To replace seeded staging data, use a separately reviewed reset procedure. Normal start/stop deliberately does not overwrite or delete an initialized named volume.
